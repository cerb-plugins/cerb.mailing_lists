{$page_context = CerberusContexts::CONTEXT_MAILING_LIST_MEMBER}
...
<div class="cerb-profile-toolbar">
	<form class="toolbar" action="{devblocks_url}{/devblocks_url}" onsubmit="return false;" style="margin-bottom:5px;">
		...
		<!-- Macros -->
		{devblocks_url assign=return_url full=true}c=profiles&type=mailing_list_member&id={$page_context_id}-{$mailing_list_member->name|devblocks_permalink}{/devblocks_url}
		{include file="devblocks:cerberusweb.core::internal/macros/display/button.tpl" context=$page_context context_id=$page_context_id macros=$macros return_url=$return_url}
		
		<!-- Edit -->
		<button type="button" id="btnDisplayMailingListMemberEdit" title="{'common.edit'|devblocks_translate|capitalize}">&nbsp;<span class="cerb-sprite2 sprite-gear"></span>&nbsp;</button>
	</form>
...
<fieldset class="properties">
	<legend>{'Mailing List Member'|devblocks_translate|capitalize}</legend>
...
<div id="mailing_list_memberTabs">
	<ul>
		...
		<li><a href="{devblocks_url}ajax.php?c=profiles&a=handleSectionAction&section=mailing_list_member&action=showBroadcastsTab&scope=target&point={$point}&context={$page_context}&context_id={$page_context_id}{/devblocks_url}">{'common.broadcasts'|devblocks_translate|capitalize} <div class="tab-badge">{''|default:0}</div></a></li>
...
<script type="text/javascript">
	$(function() {
		...
		var tabs = $("#mailing_list_memberTabs").tabs(tabOptions);
		
		$('#btnDisplayMailingListMemberEdit').bind('click', function() {
			$popup = genericAjaxPopup('peek','c=internal&a=showPeekPopup&context={$page_context}&context_id={$page_context_id}',null,false,'550');
			$popup.one('mailing_list_member_save', function(event) {
...
$(document).keypress(function(event) {
	...
	switch(event.which) {
		...
		case 58:  // (0) tab cycle
			try {
				idx = event.which-49;
				$tabs = $("#mailing_list_memberTabs").tabs();
				$tabs.tabs('option', 'active', idx);
			} catch(ex) { }
			break;
		case 101:  // (E) edit
			try {
				$('#btnDisplayMailingListMemberEdit').click();
			} catch(ex) { }
			break;
		case 109:  // (M) macros
			try {
